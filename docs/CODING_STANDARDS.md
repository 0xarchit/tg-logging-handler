# Coding Standards — `tglog-handler`
...
- **Type checker**: `mypy --strict` must pass with zero errors on the `tglog_handler/` package. Tests directory may relax strictness slightly (`disallow_untyped_defs = false` for `tests/`) but should still be typed where practical.
- **Test runner**: `pytest` with `pytest-cov`. Coverage target: ≥90% on `tglog_handler/` (excluding `__init__.py` re-exports).
...
- `handler.py` (the public `TelegramHandler` class) should be thin: constructor wiring + `emit`/`close` delegation. Business logic (batching decisions, retry math, split logic) lives in the other modules, not in the handler class itself. This keeps the `logging.Handler` subclass easy to reason about against the stdlib contract.
...
- No global mutable state. Every `TelegramHandler` instance owns its own queue, worker thread, HTTP client, and stats — verified by a test that constructs two handlers and confirms independence (see TESTING.md).
...
- Every public symbol (`TelegramHandler`, `HandlerStats`, `TelegramConfigError`) has a complete docstring matching API_SPEC.md.
...
- Every commit that touches `tglog_handler/` must leave `pytest` and `mypy --strict` green — no "fix in next commit" WIP states on the main branch.
